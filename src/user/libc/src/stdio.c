...
int open(const char *pathname, int flags, mode_t mode) {
     return __sys_open(pathname, mode | flags);
}
...
static int str_to_flags(const char *restrict mode) {
    ...
    for (; *mode; mode++) {
        switch (*mode) {
            ...
            case '+':
                ...
                break;
            case 'b':
                /* binary mode, often ignored with POSIX */
                break;
            default:
                printf("Invalid flag when opening file! Flags: %s\n", mode);
                break;
        }
    }
    ...
    if (create)   ret |= O_CREAT;
    if (truncate) ret |= O_TRUNC;
    if (append)   ret |= O_APPEND;
